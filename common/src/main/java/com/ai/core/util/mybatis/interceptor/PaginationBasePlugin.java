package com.ai.core.util.mybatis.interceptor;
...
import com.ai.core.util.dao.Page;
import com.ai.core.util.dao.PageCondition;
import com.ai.core.util.dao.PageHolder;
...
public abstract class PaginationBasePlugin implements Interceptor {
	...
	private static final Logger log = LoggerFactory
			.getLogger(PaginationBasePlugin.class);
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		final MappedStatement mappedStatement = this
				.getMappedStatement(invocation);
		...
		if (parameter instanceof PageCondition) { // 只有一个参数的情况
			...
			for (Map.Entry<Object, Object> e : ((Map<Object, Object>) parameter)
					.entrySet()) {
		...
		if (page != null) {
			...
			int totalCount = getTotalCount(sql, connection, mappedStatement,
					parameter, boundSql);
	...
	public int getTotalCount(final String sql, final Connection connection,
			final MappedStatement statement, final Object parameterObj,
		...
		try {
			...
			setParameters(pStatement,
					this.buildMappedStatement(statement, boundSql, sql),
					boundSql, parameterObj);
	...
	private MappedStatement buildMappedStatement(MappedStatement ms,
		...
		Builder builder = new MappedStatement.Builder(ms.getConfiguration(),
				ms.getId(), new BoundSqlSqlSource(ms, boundSql, sql),
	...
	public void setProperties(Properties properties) {
		...
		// } catch (Exception e) {
		// throw new
		// MyBatisShardsException("Can not create dialect instance by dialect:"
		// + dialectClass, e);
		// }
	}
